...
# Função para enviar a mensagem letra por letra
def enviar_mensagem_letra_por_letra(mensagem, intervalo=0.5):
    global enviar, parar
    for letra in mensagem:
        if parar:  # Verifica se foi solicitado parar
            print("Envio interrompido!")
            break
        if enviar:  # Verifica se o envio foi iniciado
            pyautogui.write(letra)  # Escreve a letra no campo de texto
            pyautogui.press("enter")  # Pressiona Enter para enviar
            time.sleep(intervalo)  # Espera o tempo definido entre cada letra
...
O FIM"""
    intervalo = 0.1
    enviar_mensagem_letra_por_letra(mensagem, intervalo)
...
# Listener para iniciar o envio ao pressionar a tecla "i"
def escutar_teclas():
    global enviar, parar
    while True:
        if keyboard.is_pressed('i'):  # Se a tecla 'i' for pressionada, inicia o envio
            if not enviar:
                print("Envio iniciado!")
                threading.Thread(target=iniciar_envio).start()
            time.sleep(0.1)  # Evitar múltiplos envios pela mesma tecla pressionada

        if keyboard.is_pressed('p'):  # Se a tecla 'p' for pressionada, para o envio
            if enviar:
                print("Envio será interrompido!")
                parar_envio()
            time.sleep(0.1)

# Rodar o listener em um thread separado
listener_thread = threading.Thread(target=escutar_teclas)
listener_thread.daemon = True
listener_thread.start()

# Manter o programa rodando
print("Pressione 'i' para iniciar e 'p' para parar.")
while True:
    time.sleep(1)
